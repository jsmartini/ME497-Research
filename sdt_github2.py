...
class SDT(nn.Module):
    ...
    def __init__(
        ...
        self.inner_nodes = nn.Sequential(
            nn.Linear(self.input_dim +1 , self.internal_node_num_, bias=True),
            nn.Linear(self.internal_node_num_, self.internal_node_num_, bias = True),
            nn.Sigmoid(),
        )

        self.leaf_nodes = nn.Sequential(
          nn.Linear(self.leaf_node_num_,self.output_dim,bias=False),
          nn.Softmax(),
        )
